...
def create_cooccurrence_matrix(
    W: Set[str], S: Set[str], C: List[List[str]], N: int = 5
) -> List[List[int]]:
    """
    ...
    S (Set[str]): A set of stopwords to exclude from co-occurrence counting.
    ...
    for sentence in C:
        ...
        for i, word_token in enumerate(sentence):
            if word_token in W and word_token not in coocs:
                coocs[word_token] = {}
                l = max(0, i - N)
...
if __name__ == "__main__":
    ...
    for corpus in corpora:
        ...
        M = create_cooccurrence_matrix(
            W=set(target_words.keys()), S=stop_words, C=lemmatized_sentences, N=5
        )
